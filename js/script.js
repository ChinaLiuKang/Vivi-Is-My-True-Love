...
window.checkEnter = function(e) { if (e.key === 'Enter') checkPassword(); };
...
function 显示解锁消息() {
    ...
    div.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:rgba(0,0,0,0.95);padding:3rem;border-radius:40px;border:3px solid #2ed573;text-align:center;z-index:10000;box-shadow:0 0 100px #2ed573;animation:fadeInUp 0.5s;';
    div.innerHTML = '<h2 style="color:#2ed573;font-size:2.5rem;">🎉 解锁成功！</h2><p style="margin:1rem 0;color:#fff;font-size:1.5rem;">祝学友哥与Vivi姐永世幸福</p><p style="color:#aaa;">轮回之爱系统启动</p>';
...
const lyrics = [
    ...
    // 第二段主歌（约 93~135 秒）
    { time: 93, text: "等不到你 成为我最闪亮的星星" },
    { time: 100, text: "我依然愿意借给你我的光" },
    { time: 107, text: "投射给你 直到你那灿烂的光芒" },
    { time: 114, text: "静静地挂在遥远的天上" },
    { time: 121, text: "当你沉浸 天空那条冰冷的银河" },
    { time: 128, text: "隐没在宇宙深处 我正穿越" },
    { time: 135, text: "茫茫人海 为你点亮 一盏温暖的灯火" },
    // 第二段副歌（约 149~170 秒）
    { time: 149, text: "一闪一闪亮晶晶 好像你的身体" },
    { time: 156, text: "藏在众多孤星之中 还是找得到你" },
    { time: 163, text: "挂在天上放光明 反射我的孤寂" },
    { time: 170, text: "提醒我 我也只是一颗寂寞的星星" },
    // 结束语（240秒，接近尾声）
    { time: 240, text: "🌟 歌曲已终了，轮回之爱永存" }
];
...
window.skipForward = () => { if (contentUnlocked) music.currentTime = Math.min(music.duration, music.currentTime + 15); };
window.skipBackward = () => { if (contentUnlocked) music.currentTime = Math.max(0, music.currentTime - 15); };
